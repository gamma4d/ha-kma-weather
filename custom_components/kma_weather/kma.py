...
def build_hourly(items: list[dict]) -> list[dict]:
    ...
    for date, time in sorted(grouped):
        ...
        if "WSD" in c:
            entry["native_wind_speed"] = float(c["WSD"])
        if "VEC" in c:
            entry["wind_bearing"] = float(c["VEC"])
...
def cap_hourly(hourly: list[dict], hours: int = 24) -> list[dict]:
    """Limit an hourly forecast (1h steps, sorted) to the next `hours` entries."""
    return hourly[:hours]


def build_daily(items: list[dict]) -> list[dict]:
    """단기예보 items -> per-day HA forecast (TMX/TMN, max POP, day's PCP sum, condition)."""
    grouped = _group_by_time(items)
    days: dict[str, dict] = {}
    for (date, time), c in grouped.items():
        d = days.setdefault(
            date, {"pops": [], "tmx": None, "tmn": None, "precip": None, "slots": {}}
        )
        if "POP" in c:
        ...
        if "TMN" in c:
            d["tmn"] = float(c["TMN"])
        if "PCP" in c:
            d["precip"] = (d["precip"] or 0.0) + parse_amount(c["PCP"])
        d["slots"][time] = c
    out: list[dict] = []
    for date in sorted(days):
        ...
        if d["precip"] is not None:
            entry["native_precipitation"] = round(d["precip"], 1)
...
def build_current(ncst_items: list[dict], fcst_items: list[dict], now: datetime) -> dict:
    ...
    wsd = _ncst_val(ncst_items, "WSD")
    if wsd is not None:
        cur["wind_speed"] = float(wsd)
    vec = _ncst_val(ncst_items, "VEC")
    if vec is not None:
        cur["wind_bearing"] = float(vec)
